// ✅ app-main.js — CrimznBot Full Logic + Wallet Connect

// 🆕 Define connectWallet for binding
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function checkPaymentStatus() {
    ...
    try {
      const res = await fetch(`/api/verify`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ wallet: window.connectedWallet }) // ✅ global wallet
      });
  ...
  // 💰 Solana Pay button — opens Solana Pay with preset amount
  document.getElementById("solana-pay-btn").addEventListener("click", () => {
    ...
    const url = `https://solana.com/pay/${address}?amount=0.025&label=${encodeURIComponent(label)}&message=${encodeURIComponent(message)}`;
