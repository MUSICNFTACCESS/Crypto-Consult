  ...
  try {
    ...
    return data.some(tx =>
      ...
      tx.nativeTransfers?.some(t =>
        t.to === SOLANA_ADDRESS && parseFloat(t.amount) >= 0.025
      )
    );
  } catch (err) {
...
app.post("/api/crimznbot", async (req, res) => {
  ...
  try {
    ...
    const btc = priceData?.bitcoin?.usd ?? "Unavailable";
    const eth = priceData?.ethereum?.usd ?? "Unavailable";
    const sol = priceData?.solana?.usd ?? "Unavailable";
    ...
    const tokenAliases = {
      ...
      pepe: "pepe", people: "constitutiondao", ltc: "litecoin", ada: "cardano",
      kas: "kaspa", injective: "injective-protocol", sui: "sui", avax: "avalanche-2",
      rndr: "render-token", ksm: "kusama", rose: "oasis-network", stx: "stacks",
      beam: "onbeam", kda: "kadena", one: "harmony", icp: "internet-computer",
      route: "router-protocol", aleph: "aleph-im", rdk: "rdk", dogwifhat: "dogwifcoin"
    };
    ...
    const aiRes = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You are CrimznBot — a fearless crypto strategist blending Raoul Pal, Michael Saylor, and a crypto degen. You know live token prices."
          },
...
app.post("/api/pulseit", async (req, res) => {
  ...
  try {
    ...
    return res.json({ response: "❌ PulseIt failed to analyze sentiment." });
  }
});
